...
describe('sessions that cannot stall', () => {
  it.each(['ready', 'failed', 'merged', 'needs_input', 'starting', 'stalled'] as const)(
    'never fires for a session in %s',
    (runtimeState) => {
      // needs_input is waiting on the operator, not stuck; the terminal states
      // are finished; stalled has already fired.
      expect(evaluateStall(facts({ runtimeState }), DEFAULT_THRESHOLDS, 60 * MIN)).toBeNull()
    }
  )
})
