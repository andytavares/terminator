#!/usr/bin/env bash
# claude-harness installer / updater / uninstaller
# Usage:
#   ./harness.sh                   # interactive menu
#   ./harness.sh install <target>  # install into <target>
#   ./harness.sh update  <target>  # update existing install in <target>
#   ./harness.sh uninstall <target>
#   ./harness.sh status  <target>  # show what's installed and version
#   ./harness.sh --help

set -euo pipefail

VERSION="0.1.0"
ASSUME_YES="${HARNESS_YES:-0}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MANIFEST_NAME=".claude-harness-manifest.json"
...
backup_dir() {
  printf '%s/.claude-harness-backups/%s' "$1" "$(date +%Y%m%d-%H%M%S)"
}
...
cmd_install() {
  ...
  info "Installing claude-harness $VERSION into $target"
  say  ""

  # Guard against existing install
  if [ -f "$target/$MANIFEST_NAME" ]; then
    local cur; cur="$(jq -r .harness_version < "$target/$MANIFEST_NAME" 2>/dev/null || echo unknown)"
    warn "harness already installed (version $cur)"
  ...
  cp "$SCRIPT_DIR/scripts/harness.sh"               "$target/scripts/harness.sh"
  ...
  say "  3. Run: cd ${target} && claude  →  try /detect-stack, /ask, /tdd, /review"
}
...
cmd_update() {
  ...
  say "  Roll back with: ./scripts/harness.sh restore $target $bdir"
}
...
cmd_uninstall() {
  ...
  warn "About to UNINSTALL the harness from $target"
  say ""
...
cmd_status() {
  local target; target="$(resolve_target "${1:-}")" || return 1
  if [ ! -f "$target/$MANIFEST_NAME" ]; then
    warn "no harness installed at $target"
    return 1
  fi
  info "Harness status at $target"
...
cmd_restore() {
  ...
  [ -z "$target" ] || [ -z "$bdir" ] && { err "usage: harness.sh restore <target> <backup-dir>"; return 1; }
...
cmd_help() {
  cat <<HELP
${BOLD}claude-harness $VERSION${NC}

A cookiecutter Claude harness for large multi-language codebases.

${BOLD}USAGE${NC}
  $(basename "$0")                          interactive menu
  $(basename "$0") install   <target-repo>  install into <target-repo>
  $(basename "$0") update    <target-repo>  update existing install
  $(basename "$0") uninstall <target-repo>  remove the harness
...
${BOLD}FLAGS${NC}
  -y, --yes                          auto-accept all prompts (CI mode)
                                      also via HARNESS_YES=1 env var
...
${BOLD}WHAT IS MANAGED${NC}
  ...
  scripts/detect-stack.sh, scripts/harness.sh.
...
${BOLD}BACKUPS${NC}
  ...
  .claude-harness-backups/<timestamp>/ inside the target repo.
...
interactive_menu() {
  say "${BOLD}claude-harness $VERSION${NC}"
  say ""
