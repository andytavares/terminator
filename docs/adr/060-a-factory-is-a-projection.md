...
- Idling has exactly one place: the lounge. A node whose state is `waiting`, `passed`, `skipped` or `blocked` sends its crew member there. Nothing walks the production floor without a cause.
