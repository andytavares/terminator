  ...
  return {
    startPhaseRunner(opts) {
      ...
      child.on('error', (err) => {
        ...
        api.window.broadcast('foundry:run-phase-complete', { featureDir, phase, exitCode: 1 })
      })
      ...
      child.on('close', (exitCode) => {
        ...
        if (phase === 'implement' && batchIndex !== undefined) {
          api.window.broadcast('foundry:checkin-ready', {
            featureDir,
            batchIndex,
            diffSummary: outputBuffer.join('').slice(-500),
          })
        } else {
          api.window.broadcast('foundry:run-phase-complete', {
            featureDir,
            phase,
            exitCode: code,
          })
  ...
  void (async () => {
    ...
    const phaseCallbacks = {
      onPending: (pending: PendingPermission) => {
        ...
        opts.api.window.broadcast('foundry:permission-requested', {
      ...
      onResolved: (requestId: string, decision: PermissionOutcome) => {
        ...
        opts.api.window.broadcast('foundry:permission-resolved', {
          featureDir: opts.featureDir,
          requestId,
          decision,
        })
