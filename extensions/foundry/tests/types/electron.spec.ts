...
describe('getSpeckitAPI()', () => {
  ...
  it('calls bridge.invoke with speckit:file-write', async () => {
    ...
    expect(mockInvoke).toHaveBeenCalledWith('foundry:file-write', {
  ...
  it('calls bridge.invoke with speckit:pilot-state', async () => {
    ...
    expect(mockInvoke).toHaveBeenCalledWith('foundry:pilot-state', {
  ...
  it('calls bridge.invoke with speckit:phase-approve', async () => {
    ...
    expect(mockInvoke).toHaveBeenCalledWith('foundry:phase-approve', {
      featureDir: '/repo/specs/001',
      phase: 'specify',
      note: 'LGTM',
    })
  ...
  it('calls bridge.invoke with speckit:phase-revoke', async () => {
    ...
    expect(mockInvoke).toHaveBeenCalledWith('foundry:phase-revoke', {
      featureDir: '/repo/specs/001',
      phase: 'plan',
    })
  })
  ...
  it('calls bridge.invoke with speckit:artifact-read', async () => {
    ...
    expect(mockInvoke).toHaveBeenCalledWith('foundry:artifact-read', {
      featureDir: '/repo/specs/001',
      filePath: 'plan.md',
    })
  })
  ...
  it('calls bridge.invoke with speckit:history-load', async () => {
    ...
    expect(mockInvoke).toHaveBeenCalledWith('foundry:history-load', {
      featureDir: '/repo/specs/001',
    })
  })
  ...
  it('calls bridge.on for onStateChanged and returns unsub', () => {
    ...
    expect(mockOn).toHaveBeenCalledWith('foundry:state-changed', handler)
