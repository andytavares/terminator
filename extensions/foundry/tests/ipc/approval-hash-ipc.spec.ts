...
describe('approving a phase', () => {
  it('hashes the artifact the phase wrote, not the path it was recorded under', async () => {
    ...
    const { state } = (await call('foundry:phase-approve', {
      featureDir,
      phase: 'specify',
    })) as { state: PilotState }
    ...
    rmSync(specPath)
    const { state: second } = (await call('foundry:phase-approve', {
      featureDir,
      phase: 'specify',
    })) as { state: PilotState }
...
describe('reading the card afterwards', () => {
  it('says nothing while the artifact is what was approved', async () => {
    await call('foundry:phase-approve', { featureDir, phase: 'specify' })
    const { state } = (await call('foundry:pilot-state', { featureDir })) as { state: PilotState }
  ...
  it('does not write the verdict back, so undoing the edit undoes it', async () => {
    ...
    await call('foundry:pilot-state', { featureDir })
...
describe('the gate preview', () => {
  it('shows the artifact the phase wrote, from the worktree it wrote it in', async () => {
    ...
    const result = (await call('foundry:artifact-read', {
...
describe('skipping a phase', () => {
  it('skips it, and lets it back', async () => {
    const skipped = (await call('foundry:phase-skip', { featureDir, phase: 'checklist' })) as {
      state: PilotState
    }
    expect(skipped.state.phases.checklist.status).toBe('skipped')

    const restored = (await call('foundry:phase-unskip', { featureDir, phase: 'checklist' })) as {
      state: PilotState
    }
