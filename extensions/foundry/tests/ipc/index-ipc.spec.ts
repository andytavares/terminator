...
describe('foundry:card-list', () => {
  it('registers the handler', () => {
    expect(getSharedHandler('foundry:card-list')).toBeDefined()
  })

  it('requires repoRoot', async () => {
    const handler = getSharedHandler('foundry:card-list')!
...
describe('foundry:card-create', () => {
  it('rejects an empty title with VALIDATION_ERROR', async () => {
    const handler = getSharedHandler('foundry:card-create')!
...
describe('foundry:card-update', () => {
  it('requires featureDir', async () => {
    const handler = getSharedHandler('foundry:card-update')!
...
describe('foundry:card-comment and comment-list', () => {
  it('rejects an empty comment', async () => {
    const handler = getSharedHandler('foundry:card-comment')!
...
describe('foundry:card-move', () => {
  it('rejects an unknown stage', async () => {
    const handler = getSharedHandler('foundry:card-move')!
    const result = (await handler({
      featureDir: '/repo/specs/x',
      workspacePath: '/repo',
      toStage: 'nope',
  ...
  it('sets the stage to any target without starting a run', async () => {
    ...
    const broadcast = vi
      ...
      .mock.calls.filter((c) => c[0] === 'foundry:state-changed')
...
describe('foundry:card-handoff', () => {
  it('starts a backlog card immediately when under the cap', async () => {
    ...
    const handler = getSharedHandler('foundry:card-handoff')!
    const result = (await handler({
      featureDir: '/repo/specs/x',
      workspacePath: '/repo',
  ...
  it('quick mode skips the SpecKit phases and starts at plan from the ticket', async () => {
    ...
    const stateChange = vi
      .mocked(sharedApi.window.broadcast)
      .mock.calls.find((c) => c[0] === 'foundry:state-changed')
    const state = (
...
describe('foundry:card-reset', () => {
  it('requires featureDir', async () => {
    const handler = getSharedHandler('foundry:card-reset')!
...
describe('foundry:run-reply', () => {
  it('errors when there is no active conversation to reply to', async () => {
    const handler = getSharedHandler('foundry:run-reply')!
  ...
  it('resumes the captured session with the reply text', async () => {
    ...
    vi.mocked(persistence.readCard).mockResolvedValue(null)
    await getSharedHandler('foundry:card-handoff')!({
      featureDir: '/repo/specs/x',
      workspacePath: '/repo',
    })
    ...
    const result = (await getSharedHandler('foundry:run-reply')!({
      featureDir: '/repo/specs/x',
...
describe('foundry:artifact-list', () => {
  it('requires featureDir', async () => {
    const handler = getSharedHandler('foundry:artifact-list')!
...
describe('foundry:knowledge-search', () => {
  it('requires repoRoot and query', async () => {
    const handler = getSharedHandler('foundry:knowledge-search')!
...
describe('foundry:ticket-list', () => {
  it('registers the speckit:ticket-list handler', () => {
    expect(getSharedHandler('foundry:ticket-list')).toBeDefined()
  })
  ...
  it("asks the application's connection rather than owning one", async () => {
    ...
    const handler = getSharedHandler('foundry:ticket-list')!
...
describe('tracker credentials', () => {
  it('no longer offers credential channels — they belong to the application', () => {
    ...
    expect(getSharedHandler('foundry:credentials-set')).toBeUndefined()
    expect(getSharedHandler('foundry:credentials-status')).toBeUndefined()
  })
})

describe('foundry:dispatch', () => {
  const ticket = {
  ...
  it('registers the speckit:dispatch handler', () => {
    expect(getSharedHandler('foundry:dispatch')).toBeDefined()
  })

  it('returns error when ticket is missing', async () => {
    const handler = getSharedHandler('foundry:dispatch')!
...
describe('foundry:run-cancel', () => {
  const featureDir = '/repo/specs/001-test'
  const workspacePath = '/repo'

  it('registers the speckit:run-cancel handler', () => {
    expect(getSharedHandler('foundry:run-cancel')).toBeDefined()
  })

  it('returns error when featureDir is missing', async () => {
    const handler = getSharedHandler('foundry:run-cancel')!
    const result = (await handler({ workspacePath })) as { error?: string }
    expect(result.error).toBeDefined()
  })
...
describe('foundry:open-pr', () => {
  ...
  it('returns error when gh auth is not configured', async () => {
    ...
    const handler = getSharedHandler('foundry:open-pr')!
