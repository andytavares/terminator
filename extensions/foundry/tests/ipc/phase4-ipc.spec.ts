...
describe('foundry:self-review-read', () => {
  it('registers the handler', () => {
    expect(getSharedHandler('foundry:self-review-read')).toBeDefined()
  })

  it('returns error when featureDir is missing', async () => {
    const handler = getSharedHandler('foundry:self-review-read')!
...
describe('foundry:phase-request-changes', () => {
  it('registers the handler', () => {
    expect(getSharedHandler('foundry:phase-request-changes')).toBeDefined()
  })

  it('returns error when featureDir is missing', async () => {
    const handler = getSharedHandler('foundry:phase-request-changes')!
  ...
  it('broadcasts speckit:state-changed after update', async () => {
    ...
    expect(sharedApi.window.broadcast).toHaveBeenCalledWith(
      'foundry:state-changed',
      expect.objectContaining({ state: expect.anything() })
    )
  })
})

describe('foundry:phase-comment', () => {
  it('registers the handler', () => {
    expect(getSharedHandler('foundry:phase-comment')).toBeDefined()
  })

  it('returns error when featureDir is missing', async () => {
    const handler = getSharedHandler('foundry:phase-comment')!
...
describe('foundry:checkin-decision', () => {
  it('registers the handler', () => {
    expect(getSharedHandler('foundry:checkin-decision')).toBeDefined()
  })

  it('returns error when featureDir is missing', async () => {
    const handler = getSharedHandler('foundry:checkin-decision')!
