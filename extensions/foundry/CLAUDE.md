...
Declared in `extensions/foundry/package.json` only. npm workspaces hoist them. Current extension-owned dependencies: `zod` (schema validation at every boundary), `js-yaml` (recipes, roles and rules are hand-authored and need comments), `diff`, `marked`, `@dnd-kit/*`, `@terminator/extension-ui`.
