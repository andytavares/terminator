...
- [ ] T082a **NEEDS A HUMAN** Verify SC-005: with 10 stale sessions listed, clear them all and count interactions — at most 3 (open Stale view, shift-click range, confirm), under 30 seconds
- [ ] T082b **NEEDS A HUMAN** Verify SC-002: with 20+ sessions, locate a named session twice — once by search, once by view switch — timing both at under 10 seconds with nothing expanded
...
- [ ] T085 **NEEDS A HUMAN** Run the full manual pass in [quickstart.md](./quickstart.md) Gate 5 — all 20 checks against the running app via `npm run dev`, with particular attention to items 5-10 (extension surfaces, including the footer item **actually toggling** the Git panel) and 13 (worktree removal naming the exact path)
