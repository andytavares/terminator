...
| `foundry:run.stop`             | `{ id: string }`                          | `{ ok: true } \| { error }` — ends any live agent on the run, saying why, and cancels the order. The worktrees and their changes are left alone; `run.reset` and `order.delete` are the two that take them away.                                                                                                  |
| `foundry:run.reset`            | `{ id: string }`                          | `{ ok: true, removed: string[], failed: string[] } \| { error }` — stops every live agent, destroys every lane's checkout and branch plus the graph, the gates and the rung outputs, and returns the order to `agreed` so it can run again from node zero. The order, its criteria, its plan and its ledger stay. |
| `foundry:order.delete`         | `{ id: string }`                          | `{ ok: boolean, removed: string[], failed: string[] } \| { error }` — `run.reset`'s teardown, and then the order's whole directory including its ledger. `order.cancel` is the version that keeps the record.                                                                                                     |
...
| Channel                                | Payload | Response      |
| -------------------------------------- | ------- | ------------- |
| `foundry:ui.consume-pending-new-order` | `{}`    | `{ pending }` |

### Factory view (Forge surface projection)

Per-operator preference (`terminator.foundry.view`, `'list' \| 'factory'`, default `'list'`). Only the Forge surface's order list and Line change; Inbox, Ledger and Settings are unaffected. The `toggle-view` quick action (category `Foundry`, mnemonic `v`) flips the setting, brings Foundry forward and broadcasts the change.

| Channel                   | Payload                                                        | Response                                                                                                                                                                   |
| ------------------------- | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `foundry:ui.view`         | `{}`                                                           | `{ view: 'list' \| 'factory' }`                                                                                                                                            |
| `foundry:ui.set-view`     | `{ view: 'list' \| 'factory' }`                                | `{ view }` or `{ error }`                                                                                                                                                  |
| `foundry:ui.view-changed` | `{ view: 'list' \| 'factory' }` _(broadcast, main → renderer)_ | —                                                                                                                                                                          |
| `foundry:run.activity`    | `{ id: string }`                                               | `{ activity: Record<nodeId, ToolActivity[]> }` (last 20 per node with a session) or `{ error }`                                                                            |
| `foundry:run.timeline`    | `{ id: string }`                                               | `{ graph: RunGraph, timeline: { frames, tools }, gates: Gate[] }` — the recording in `run-timeline.jsonl` beside the graph, for the Factory view's replay — or `{ error }` |
