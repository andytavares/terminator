...
import type { BatchItem25, BatchFilter25, BatchResult25, Priority } from './types'
import { sortByPriority, filterItems, paginate, createItem25 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch25 {
  result: BatchResult25 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter25) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch25(page = 1): UseBatch25 {
  const [result, setResult] = useState<BatchResult25 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem25[]>([])

  const fetch = useCallback(async (filter: BatchFilter25 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem25(items.current.length, priority)]
  }, [])
