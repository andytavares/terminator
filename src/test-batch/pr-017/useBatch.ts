...
import type { BatchItem17, BatchFilter17, BatchResult17, Priority } from './types'
import { sortByPriority, filterItems, paginate, createItem17 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch17 {
  result: BatchResult17 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter17) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch17(page = 1): UseBatch17 {
  const [result, setResult] = useState<BatchResult17 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem17[]>([])

  const fetch = useCallback(async (filter: BatchFilter17 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem17(items.current.length, priority)]
  }, [])
