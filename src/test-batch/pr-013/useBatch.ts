...
import type { BatchItem13, BatchFilter13, BatchResult13, Priority } from './types'
import { sortByPriority, filterItems, paginate, createItem13 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch13 {
  result: BatchResult13 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter13) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch13(page = 1): UseBatch13 {
  const [result, setResult] = useState<BatchResult13 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem13[]>([])

  const fetch = useCallback(async (filter: BatchFilter13 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem13(items.current.length, priority)]
  }, [])
