...
import { sortByPriority, filterItems, paginate, createItem24 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch24 {
  result: BatchResult24 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter24) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch24(page = 1): UseBatch24 {
  const [result, setResult] = useState<BatchResult24 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem24[]>([])

  const fetch = useCallback(async (filter: BatchFilter24 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem24(items.current.length, priority)]
  }, [])
