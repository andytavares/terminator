...
import type { BatchItem31, BatchFilter31, BatchResult31, Priority } from './types'
import { sortByPriority, filterItems, paginate, createItem31 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch31 {
  result: BatchResult31 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter31) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch31(page = 1): UseBatch31 {
  const [result, setResult] = useState<BatchResult31 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem31[]>([])

  const fetch = useCallback(async (filter: BatchFilter31 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem31(items.current.length, priority)]
  }, [])
