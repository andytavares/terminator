...
import type { BatchItem30, BatchFilter30, BatchResult30, Priority } from './types'
import { sortByPriority, filterItems, paginate, createItem30 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch30 {
  result: BatchResult30 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter30) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch30(page = 1): UseBatch30 {
  const [result, setResult] = useState<BatchResult30 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem30[]>([])

  const fetch = useCallback(async (filter: BatchFilter30 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem30(items.current.length, priority)]
  }, [])
