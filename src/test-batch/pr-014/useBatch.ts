...
import type { BatchItem14, BatchFilter14, BatchResult14, Priority } from './types'
import { sortByPriority, filterItems, paginate, createItem14 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch14 {
  result: BatchResult14 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter14) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch14(page = 1): UseBatch14 {
  const [result, setResult] = useState<BatchResult14 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem14[]>([])

  const fetch = useCallback(async (filter: BatchFilter14 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem14(items.current.length, priority)]
  }, [])
