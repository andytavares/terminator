...
import type { BatchItem6, BatchFilter6, BatchResult6, Priority } from './types'
import { sortByPriority, filterItems, paginate, createItem6 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch6 {
  result: BatchResult6 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter6) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch6(page = 1): UseBatch6 {
  const [result, setResult] = useState<BatchResult6 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem6[]>([])

  const fetch = useCallback(async (filter: BatchFilter6 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem6(items.current.length, priority)]
  }, [])
