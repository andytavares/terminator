...
import type { BatchItem2, BatchFilter2, BatchResult2, Priority } from './types'
import { sortByPriority, filterItems, paginate, createItem2 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch2 {
  result: BatchResult2 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter2) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch2(page = 1): UseBatch2 {
  const [result, setResult] = useState<BatchResult2 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem2[]>([])

  const fetch = useCallback(async (filter: BatchFilter2 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem2(items.current.length, priority)]
  }, [])
