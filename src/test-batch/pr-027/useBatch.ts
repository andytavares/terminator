...
import type { BatchItem27, BatchFilter27, BatchResult27, Priority } from './types'
import { sortByPriority, filterItems, paginate, createItem27 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch27 {
  result: BatchResult27 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter27) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch27(page = 1): UseBatch27 {
  const [result, setResult] = useState<BatchResult27 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem27[]>([])

  const fetch = useCallback(async (filter: BatchFilter27 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem27(items.current.length, priority)]
  }, [])
