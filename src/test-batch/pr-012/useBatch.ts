...
import type { BatchItem12, BatchFilter12, BatchResult12, Priority } from './types'
import { sortByPriority, filterItems, paginate, createItem12 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch12 {
  result: BatchResult12 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter12) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch12(page = 1): UseBatch12 {
  const [result, setResult] = useState<BatchResult12 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem12[]>([])

  const fetch = useCallback(async (filter: BatchFilter12 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem12(items.current.length, priority)]
  }, [])
