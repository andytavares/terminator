...
export function sortByPriority(items: BatchItem5[]): BatchItem5[] {
  return [...items].sort(
...
export function paginate(items: BatchItem5[], page: number, pageSize: number): BatchResult5 {
...
export function createItem5(index: number, priority: Priority = 'medium'): BatchItem5 {
  return {
    id: `item-batch-5-${index}-${Date.now()}`,
    index,
    priority,
    tags: [`batch-5`, `index-${index}`],
    createdAt: new Date(),
    metadata: { batchId: 5, sequence: index },
  }
}
