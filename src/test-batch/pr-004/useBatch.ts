...
import type { BatchItem4, BatchFilter4, BatchResult4, Priority } from './types'
import { sortByPriority, filterItems, paginate, createItem4 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch4 {
  result: BatchResult4 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter4) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch4(page = 1): UseBatch4 {
  const [result, setResult] = useState<BatchResult4 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem4[]>([])

  const fetch = useCallback(async (filter: BatchFilter4 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem4(items.current.length, priority)]
  }, [])
