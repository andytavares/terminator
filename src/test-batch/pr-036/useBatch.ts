...
import type { BatchItem36, BatchFilter36, BatchResult36, Priority } from './types'
import { sortByPriority, filterItems, paginate, createItem36 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch36 {
  result: BatchResult36 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter36) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch36(page = 1): UseBatch36 {
  const [result, setResult] = useState<BatchResult36 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem36[]>([])

  const fetch = useCallback(async (filter: BatchFilter36 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem36(items.current.length, priority)]
  }, [])
