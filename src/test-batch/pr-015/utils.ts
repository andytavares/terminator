...
export function sortByPriority(items: BatchItem15[]): BatchItem15[] {
  return [...items].sort(
...
export function paginate(items: BatchItem15[], page: number, pageSize: number): BatchResult15 {
...
export function createItem15(index: number, priority: Priority = 'medium'): BatchItem15 {
  return {
    id: `item-batch-15-${index}-${Date.now()}`,
    index,
    priority,
    tags: [`batch-15`, `index-${index}`],
    createdAt: new Date(),
    metadata: { batchId: 15, sequence: index },
  }
}
