...
export function sortByPriority(items: BatchItem40[]): BatchItem40[] {
  return [...items].sort(
...
export function paginate(items: BatchItem40[], page: number, pageSize: number): BatchResult40 {
...
export function createItem40(index: number, priority: Priority = 'medium'): BatchItem40 {
  return {
    id: `item-batch-40-${index}-${Date.now()}`,
    index,
    priority,
    tags: [`batch-40`, `index-${index}`],
    createdAt: new Date(),
    metadata: { batchId: 40, sequence: index },
  }
}
