...
import type { BatchItem50, BatchFilter50, BatchResult50, Priority } from './types'
import { sortByPriority, filterItems, paginate, createItem50 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch50 {
  result: BatchResult50 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter50) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch50(page = 1): UseBatch50 {
  const [result, setResult] = useState<BatchResult50 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem50[]>([])

  const fetch = useCallback(async (filter: BatchFilter50 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem50(items.current.length, priority)]
  }, [])
