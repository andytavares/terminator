...
import { sortByPriority, filterItems, paginate, createItem22 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch22 {
  result: BatchResult22 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter22) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch22(page = 1): UseBatch22 {
  const [result, setResult] = useState<BatchResult22 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem22[]>([])

  const fetch = useCallback(async (filter: BatchFilter22 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem22(items.current.length, priority)]
  }, [])
