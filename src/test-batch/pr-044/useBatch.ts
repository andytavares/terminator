...
import type { BatchItem44, BatchFilter44, BatchResult44, Priority } from './types'
import { sortByPriority, filterItems, paginate, createItem44 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch44 {
  result: BatchResult44 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter44) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch44(page = 1): UseBatch44 {
  const [result, setResult] = useState<BatchResult44 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem44[]>([])

  const fetch = useCallback(async (filter: BatchFilter44 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem44(items.current.length, priority)]
  }, [])
