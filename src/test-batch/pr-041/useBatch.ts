...
import type { BatchItem41, BatchFilter41, BatchResult41, Priority } from './types'
import { sortByPriority, filterItems, paginate, createItem41 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch41 {
  result: BatchResult41 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter41) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch41(page = 1): UseBatch41 {
  const [result, setResult] = useState<BatchResult41 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem41[]>([])

  const fetch = useCallback(async (filter: BatchFilter41 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem41(items.current.length, priority)]
  }, [])
