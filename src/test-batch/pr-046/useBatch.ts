...
import type { BatchItem46, BatchFilter46, BatchResult46, Priority } from './types'
import { sortByPriority, filterItems, paginate, createItem46 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch46 {
  result: BatchResult46 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter46) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch46(page = 1): UseBatch46 {
  const [result, setResult] = useState<BatchResult46 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem46[]>([])

  const fetch = useCallback(async (filter: BatchFilter46 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem46(items.current.length, priority)]
  }, [])
