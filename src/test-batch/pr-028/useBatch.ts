...
import type { BatchItem28, BatchFilter28, BatchResult28, Priority } from './types'
import { sortByPriority, filterItems, paginate, createItem28 } from './utils'
import { DEFAULT_PAGE_SIZE, MAX_ITEMS, RETRY_LIMIT, TIMEOUT_MS } from './constants'

interface UseBatch28 {
  result: BatchResult28 | null
  loading: boolean
  error: string | null
  fetch: (filter?: BatchFilter28) => Promise<void>
  reset: () => void
  addItem: (priority?: Priority) => void
}

export function useBatch28(page = 1): UseBatch28 {
  const [result, setResult] = useState<BatchResult28 | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const retries = useRef(0)
  const items = useRef<BatchItem28[]>([])

  const fetch = useCallback(async (filter: BatchFilter28 = {}) => {
    setLoading(true)
    setError(null)
    try {
  ...
  const addItem = useCallback((priority: Priority = 'medium') => {
    ...
    items.current = [...items.current, createItem28(items.current.length, priority)]
  }, [])
