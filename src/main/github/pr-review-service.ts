import type { Chapter, PrChangedFile, RiskScore, FileMetrics, ReviewQueuePR } from '../../shared/schemas/pr-review.schema.js'
...
// ─── Complexity hotspot detection (FR-021) ───────────────────────────────────
...
// ─── Risk score computation (FR-016, FR-017) ─────────────────────────────────
...
export function computeRiskScore(
  ...
  // Min-max normalise each numeric metric within the current PR file set
  ...
  const coverage_n = metrics.patchCoverage != null
    ? 1 - (metrics.patchCoverage / 100) // low coverage = high risk
    : null

  // Weighted composite (weights sum to 1 before coverage)
  // Base weights: churn 0.25, blast 0.25, size 0.20, complexity 0.10, coverage 0.05, test 0.15
  ...
  if (available.length >= 2) {
    ...
    composite = (weighted / totalWeight) * 80  // scale to 0-80; test penalty adds up to 20

    if (!metrics.testFilePresent) {
      composite += MISSING_TEST_PENALTY
    }
    composite = Math.min(100, Math.round(composite))
  }

  // Dominant driver — highest contributing metric
...
// ─── Chapter building (FR-007, FR-008, FR-015) ───────────────────────────────
...
export function buildChapters(
  ...
  // Group by first differing path segment
  ...
  // Build tiers per group, then one chapter per group (or four tier-chapters if single group)
  ...
  const buildGroupChapters = (groupKey: string, groupFiles: RawFile[]) => {
    ...
    // Single chapter per group containing all tiers in order
    ...
    const isMechanical = ordered.every(f => f.tier === 3)
    chapters.push({
      id:               chapterId,
  ...
  // Separate mechanical-only groups to put them last
...
export function parseReviewQueuePR(raw: unknown): ReviewQueuePR {
  ...
  return {
    ...
    riskLevel:          'low',  // updated lazily via fileMetrics calls
    signalDots: {
      tests:    'unknown',
      coverage: 'unknown',
      ci:       'unknown',
...
// ─── Force-push changed-file detection (FR-026) ───────────────────────────────
...
export function detectChangedFiles(
  ...
  for (const newFile of newFiles) {
    ...
    if (!old) {
      ...
      // Use size change as proxy for content change (SHA not available from gh pr view files)
      changed.add(newFile.path)
    }
  }
  return changed
