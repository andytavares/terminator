...
- **SpecKit Pilot** — A Quill-style workflow board for controlling feature implementation end-to-end when offloading to agents. The home surface is a kanban board with six stage columns (Backlog → Spec → Plan → Implement → In Review → Done); every piece of work is a **card** that carries a brief (title, type, scope, checklist). Author cards natively in the board or import them from Linear/Jira; hand a card off and an agent works it through the 10-phase Spec-Kit pipeline (Constitution → Specify → Clarify → Plan → Checklist → Tasks → Analyze → Implement → Self-review → Open PR) in its own git worktree, with human approval gates at every phase. **Multiple cards run in parallel** up to a configurable `maxConcurrentRuns` cap; excess hand-offs queue and start automatically as slots free. Opening a card reveals a detail drawer with tabs: **Brief** (edit), **Phases** (the phase rail + streaming console, toggleable between plain-text and markdown rendering, + gates), **Activity** (comments + audit log — a comment steers the agent's next phase), and **Artifacts** (spec/plan/tasks/diff/self-review/PR with git revision history). A workspace **knowledge search** (keyword) finds repo docs and briefs and attaches them to a card as agent context. State persists to `.pilot/state.json` (v3), the brief to `.pilot/card.json`, comments to `.pilot/comments.jsonl`, and the audit log to `.pilot/history.jsonl` inside each feature dir. Credentials (Linear/Jira API keys) stay in the main-process secrets store, never exposed to the renderer.
