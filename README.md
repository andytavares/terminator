...
- **Session views** — The sidebar is a flat list of every session, not a tree you have to unfold. A view decides how it is grouped (workspace — the default, which nests each project under its workspace so a branch is still one click from a new terminal — project, status, branch, or not at all), how it is sorted, and what is filtered out. A project header names the workspace it belongs to whenever the workspace has no header of its own. Four views ship built in — **Everything**, **Needs me**, **Active**, **Stale** — and you can save your own; they survive a restart. Whenever a filter hides anything the sidebar says so ("showing 6 of 22") with a one-click way back, and it always opens on the unfiltered Everything view. Each row shows how long since the session last did anything, an optional one-line note (`⌘I`), and its state. `⌘K` lists sessions alongside commands, `⌘]` / `⌘[` cycle recently used sessions across projects, and `⌘⇧A` jumps to the next session waiting on you.
