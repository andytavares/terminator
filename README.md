...
- **Agent Supervision Console** — Supervises a small number of long-running Claude Code agents, on the premise that review capacity is the bottleneck and the failure nobody instruments is an agent that is stuck without asking. Every supervised session carries a **derived runtime state** (`starting`, `working`, `needs_input`, `stalled`, `ready`, `failed`, `merged`, `unknown`) observed from the agent's own activity — never by parsing terminal output — so a blocked agent is visible in under two seconds without opening it. A **stall detector** runs every 30 seconds on three signals (silence, looping on one file with no net change, repeated self-reverts), excluding time spent inside a long-running shell command so a twelve-minute test suite never reads as a stall. It ships in **shadow mode by default**: firings are recorded with the values that triggered them, but nothing is surfaced until you turn shadow mode off on the evidence of the precision report. Finished work enters a **review queue ordered worst-first** by risk grade, with the specific trigger shown for each; **backpressure** refuses to start a new agent while three finished sessions are unreviewed, and every override is recorded with the queue depth at the time. Unattended merge exists only for the lowest grade, per-repository, off by default, and never when checks are anything other than green. Each agent runs in its own **provisioned worktree** — heavy ignored directories shared by symlink, declared files copied, a non-overlapping port span allocated, and the repository's setup script run, with a non-zero exit surfacing as a failed session rather than a mystery inside a transcript. Configure per repository in `.terminator/config.json`.
