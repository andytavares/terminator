...
- **SpecKit Pilot** — Extension that automates the full ticket-to-PR lifecycle across a 10-phase pipeline (Constitution → Specify → Clarify → Plan → Checklist → Tasks → Analyze → Implement → Self-review → Open PR). Claude Code runs autonomously as a subprocess per phase; human approval gates protect every phase boundary. Features: 4-tab UI (Tickets / Features / Active runs / History), Linear and Jira ticket integration, mini phase-rail progress indicator per feature row, batch check-in banners during Implement, self-review gate (format + lint + coverage + `/google-review` quality summary), OpenPR gate with PR title input. State persisted to `.pilot/state.json` inside each feature dir; audit log in `.pilot/history.json`. Credentials (Linear/Jira API keys) stored in main-process secrets store and never exposed to the renderer.
