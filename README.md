...
- **The board** — Click the Overview icon in the app band or press `Cmd+Shift+E` for every terminal in the app, across every repo, arranged in lanes by what it is doing: **Needs you**, **Working**, **Idle**, **Exited**. A card moves lane on its own when its terminal's state changes, and each one carries a live preview of the terminal — so you can read the question an agent is actually asking without opening it — plus its repo and branch, its CPU and memory, and how long since it was last active. An empty lane draws nothing; the exited lane is held back as history and disappears when empty. The flat grid it replaced is still there as the other layout.
