...
- **Session views** — The sidebar is a flat list of every session, not a tree you have to unfold. A view decides how it is grouped (workspace — the default, which nests each branch under its repo — branch, status, branch name, or not at all), how it is sorted, and what is filtered out. A branch row leads with its branch name, marks a worktree with its own glyph, and shows how much has changed on it; a repo header carries its folder path. Session state is a glyph — running, idle, waiting on you, exited — chosen by shape rather than colour, and selection is the row itself, so the two never compete. Four views ship built in — **Everything**, **Needs me**, **Active**, **Stale** — and you can save your own; they survive a restart. Whenever a filter hides anything the sidebar says so ("showing 6 of 22") with a one-click way back, and it always opens on the unfiltered Everything view. Each row shows how long since the session last did anything, an optional one-line note (`⌘I`), and its state. `⌘P` lists sessions alongside commands, and `⌘]` / `⌘[` cycle recently used sessions across branches. (`⌘⇧A`, "jump to the next session waiting on you", reads a stale copy of session state and does not currently find anything — a known defect, tracked separately.)
