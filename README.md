...
- **Notepad** — Markdown notes with live preview, margin comments, full-text search, tags, and import/export. **Diagrams** — embed freehand whiteboard diagrams alongside your notes using the [Excalidraw](https://excalidraw.com/) canvas (shapes, arrows, sticky notes, freehand drawing, in-place text editing by double-clicking shapes). Diagrams appear in the same note list as a distinct item type, sorted together by last-updated time. **Canvas comments** — pin Miro-style comments at any coordinate on a diagram: click the Comment button in the toolbar, click anywhere on the canvas to place a pin, write your comment, and reply or resolve threads inline. Comments follow the canvas when you zoom and pan.
